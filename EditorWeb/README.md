...
- Normalization behavior: see [SUPPORT.md](SUPPORT.md) (generated by tests).
  ...
  Raw HTML is preserved verbatim but never rendered (no HTML nodes are created),
  which is also why notes are not sanitized on load: rewriting the source would
  damage the user's bytes. The strict CSP is the second line of defence.
