...
| **I3** | P2 | **Global behavior toggles overwrite *every* per-tab behavior.** General → "Open on hover" / "Close drawer when clicking elsewhere" call `store.updateAllBehaviors` ([GeneralView.swift:70–88](MacDring/Settings/GeneralView.swift)), which rewrites that field on all tabs — silently discarding any per-tab override set in the Tabs pane. The getter's `allSatisfy` masks it (the toggle reads "off" if any tab differs). | Decide the intended model: (a) make these **new-tab defaults only** (don't touch existing tabs), matching the "New tab defaults" framing, or (b) keep the bulk apply but add a confirmation / make it clearly "apply to all tabs now." (a) is the smaller, less surprising change and matches `newTabConcealment`'s behavior. |
...
| **I5** | P3 | **Trash empty-state is home-Trash/APFS-only, but Empty Trash empties everything.** `ItemView.trashIsEmpty` infers emptiness from `~/.Trash`'s APFS link count (`== 2`) ([ItemView.swift:150–156](MacDring/Drawer/ItemView.swift)), while `FileMover.emptyTrash` tells Finder to empty *all* volumes' trashes ([FileMover.swift:48](MacDring/Launch/FileMover.swift)). The full/empty icon and the disabled "Empty Trash…" item can therefore disagree with reality (non-APFS home, or items only in external `.Trashes`). | Accept as a cosmetic heuristic and **document the limitation in a code comment**, or broaden the check to also probe per-volume `.Trashes`. Not worth a permission. |
